 ...
 * @param {IRuntime} runtime - 游戏运行时对象
 */
export function OnLayoutStrat(runtime) {
 ...
 *
 */
function PlayerMoveByWASD(runtime) {
 ...
 * @param {IRuntime} runtime - 游戏运行时对象
 */
function FixControlIntroductionText(runtime) {
    // 获取控制介绍文本实例
 ...
 * @param {IRuntime} runtime - 游戏运行时对象
 */
async function BuildingAddWall(runtime) {
    ...
    await (EventHnadlerInstance?.addEventListener)("keyPress_Num2", () => {
        if (PlayerInstance) {
            Wall2Class.createInstance("Wall", PlayerInstance.x + 10, PlayerInstance.y + 10, false);
        }
    });
    // 监听按键事件，按下数字键1时在玩家位置创建墙壁1
    await (EventHnadlerInstance?.addEventListener)("keyPress_Num1", () => {
        if (PlayerInstance) {
 ...
 * 建筑模式时高亮显示可以编辑的建筑物
 */
function BuildingModeMoveBuildingToLayer(runtime) {
    // 获取是否处于建筑模式
    const ISBuildingMode = runtime.globalVars.ISBuildingMode;
    // 获取所有建筑实例
 ...
 * 2类墙壁始终保持在最前层
 */
function BuildingWall2AWLASSetTop(runtime) {
    const ISBuildingMode = runtime.globalVars.ISBuildingMode;
    if (ISBuildingMode) {
 ...
 */
function Wall1StateSwitch(runtime) {
    const PlayerInstance = runtime.objects.player.getFirstInstance();
    const ISBuildingMode = runtime.globalVars.ISBuildingMode;
    for (let wall1 of runtime.objects.wall.instances()) {
...
function CollectableGrouptick(runtime) {
}
/**
 * 初始化可收集物品
 * @param {IRuntime} runtime - 游戏运行时对象
 */
async function CollectableInit(runtime) {
    // 获取事件处理器实例
    const EventHnadlerInstance = runtime.objects.EventHnadler.getFirstPickedInstance();
    // 鼠标悬浮在上面时改变鼠标样式
